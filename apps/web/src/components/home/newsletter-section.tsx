...
const COPY = {
  de: { eyebrow: 'Newsletter', heading: '10% auf deine erste Bestellung', body: 'Erhalte exklusive Angebote, neue Kollektionen und Style-Inspiration direkt in dein Postfach.', placeholder: 'Deine E-Mail-Adresse', cta: 'Anmelden', success: 'Willkommen! Dein Gutscheincode kommt per E-Mail.', privacy: 'Kein Spam. Jederzeit abmeldbar.' },
  en: { eyebrow: 'Newsletter', heading: '10% off your first order', body: 'Get exclusive deals, new collections, and style inspiration delivered to your inbox.', placeholder: 'Your email address', cta: 'Subscribe', success: 'Welcome! Your discount code is on its way.', privacy: 'No spam. Unsubscribe anytime.' },
  ar: { eyebrow: 'النشرة البريدية', heading: '10% خصم على طلبك الأول', body: 'احصل على عروض حصرية ومجموعات جديدة وإلهام أزياء مباشرة في بريدك.', placeholder: 'بريدك الإلكتروني', cta: 'اشترك', success: 'أهلاً! رمز الخصم في طريقه إليك.', privacy: 'لا بريد مزعج. يمكنك إلغاء الاشتراك في أي وقت.' },
}
...
export function NewsletterSection() {
  ...
  return (
    <section ref={sectionRef} className="py-20 sm:py-28 bg-ink text-white overflow-hidden relative">
      {/* Decorative gradient */}
                ...
                <div className={`relative flex-1 transition-transform duration-300 ${focused ? 'scale-[1.02]' : ''}`}>
                  <input
                    ...
                    className={`w-full h-13 px-5 rounded-full bg-white/10 border text-white text-sm placeholder:text-white/35 focus:outline-none transition-all duration-300 ${
                      focused ? 'border-brand-gold/50 ring-2 ring-brand-gold/15 bg-white/15' : 'border-white/10'
                    }`}
                  />
                </div>
                <motion.button
                  whileTap={{ scale: 0.95 }}
                  type="submit"
                  className="h-13 px-8 rounded-full bg-brand-gold hover:bg-brand-gold-dark text-white font-semibold text-sm flex items-center justify-center gap-2 transition-colors shrink-0"
                >
                  {copy.cta}
