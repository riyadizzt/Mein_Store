...
export function TrackingPixels() {
  ...
  // DSGVO: Only load marketing pixels if user consented
  const marketingConsent = useConsentStore((s) => s.marketing)

  const metaId = data?.metaPixelId
  const tiktokId = data?.tiktokPixelId

  // No consent → no pixels
...
// Pixel event helpers — check consent before firing
export function trackMetaEvent(event: string, data?: Record<string, any>) {
  if (typeof window !== 'undefined' && (window as any).fbq && useConsentStore.getState().marketing) {
    (window as any).fbq('track', event, data)
  }
}

export function trackTikTokEvent(event: string, data?: Record<string, any>) {
  if (typeof window !== 'undefined' && (window as any).ttq && useConsentStore.getState().marketing) {
