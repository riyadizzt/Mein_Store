                    ...
                    <Fragment key={p.id}><tr className="border-b hover:bg-muted/20 transition-colors group" style={{ animationDelay: `${i * 15}ms`, animation: 'fadeIn 200ms ease-out both' }}>
                      ...
                      <td className="px-3 py-3">
                        <Link href={`/${locale}/admin/products/${p.id}`} className="group/link">
                          ...
                          {(p.channelFacebook || p.channelTiktok || p.channelGoogle || p.channelWhatsapp || p.hasActiveEbayListing) && (
                            <div className="flex items-center gap-1.5 mt-1">
                              ...
                              {p.hasActiveEbayListing && (
                                <span
                                  title={locale === 'ar' ? 'نشط على eBay' : locale === 'en' ? 'Active on eBay' : 'Auf eBay aktiv'}
                                  className="inline-flex items-center justify-center"
                                >
                                  <ShoppingBag className="h-3.5 w-3.5 text-yellow-500" />
                                </span>
                              )}
                            </div>
                          )}
