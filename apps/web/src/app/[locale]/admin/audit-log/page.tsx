...
const ACTION_COLORS: Record<string, string> = {
  ...
  // C15 — channel stock-quantity sync (listener fast-path + reconcile cron)
  CHANNEL_STOCK_PUSH_FAILED: 'bg-red-200 text-red-900 dark:bg-red-600/30 dark:text-red-200',
  EBAY_STOCK_RATE_LIMITED: 'bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300',
  CHANNEL_STOCK_RECONCILE_TICK: 'bg-slate-100 text-slate-800 dark:bg-slate-500/20 dark:text-slate-300',
  // C15.1 — audit-archive cron + webhook idempotency
  AUDIT_ARCHIVE_COMPLETED: 'bg-slate-100 text-slate-800 dark:bg-slate-500/20 dark:text-slate-300',
  AUDIT_ARCHIVE_FAILED: 'bg-red-200 text-red-900 dark:bg-red-600/30 dark:text-red-200',
  EBAY_WEBHOOK_DUPLICATE: 'bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-300',
  EBAY_ACCOUNT_DELETION_RECEIVED: 'bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-300',
  // C15.1 i18n bug-fix — eBay-Listing + OAuth + Category actions
  // were missing labels and fell back to English-prettified text
  // even on AR-dashboard. Now every action emitted to admin_audit_log
  // has a 3-language entry below.
  CATEGORY_UPDATED: 'bg-slate-100 text-slate-800 dark:bg-slate-500/20 dark:text-slate-300',
  CHANNEL_LISTING_ENABLED: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300',
  EBAY_LISTING_CLEANUP_FOR_REGROUP: 'bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300',
  EBAY_LISTING_DISABLED: 'bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300',
  EBAY_LISTING_ENABLED: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300',
  EBAY_LISTING_PUBLISHED: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300',
  EBAY_LISTING_REJECTED: 'bg-red-200 text-red-900 dark:bg-red-600/30 dark:text-red-200',
  EBAY_OAUTH_CONNECT_INITIATED: 'bg-blue-100 text-blue-800 dark:bg-blue-500/20 dark:text-blue-300',
  EBAY_OAUTH_CONNECTED: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300',
  EBAY_OAUTH_DISCONNECTED: 'bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-300',
  EBAY_OAUTH_REVOKED: 'bg-red-200 text-red-900 dark:bg-red-600/30 dark:text-red-200',
  EBAY_PRODUCT_GROUP_PUBLISHED: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300',
  EBAY_PUBLISH_PENDING_BATCH: 'bg-blue-100 text-blue-800 dark:bg-blue-500/20 dark:text-blue-300',
...
const ACTION_LABELS: Record<string, { de: string; en: string; ar: string }> = {
  ...
  CHANNEL_STOCK_PUSH_FAILED: { de: 'eBay Bestand-Sync fehlgeschlagen', en: 'eBay stock sync failed', ar: 'فشل مزامنة مخزون eBay' },
  EBAY_STOCK_RATE_LIMITED: { de: 'eBay Rate-Limit beim Bestand-Sync', en: 'eBay rate-limit during stock sync', ar: 'حد معدل eBay أثناء مزامنة المخزون' },
  CHANNEL_STOCK_RECONCILE_TICK: { de: 'eBay Bestand-Reconcile Tick', en: 'eBay stock reconcile tick', ar: 'تشغيل مزامنة مخزون eBay' },
  // C15.1 — audit-archive cron + webhook idempotency
  AUDIT_ARCHIVE_COMPLETED: { de: 'Audit-Archivierung abgeschlossen', en: 'Audit archive completed', ar: 'تم اكتمال أرشفة السجل' },
  AUDIT_ARCHIVE_FAILED: { de: 'Audit-Archivierung fehlgeschlagen', en: 'Audit archive failed', ar: 'فشل أرشفة السجل' },
  EBAY_WEBHOOK_DUPLICATE: { de: 'eBay Webhook-Duplikat', en: 'eBay webhook duplicate', ar: 'إشعار eBay مكرر' },
  // i18n bug-fix — entries previously falling back to English-prettifier
  EBAY_ACCOUNT_DELETION_RECEIVED: { de: 'eBay Account-Löschung empfangen', en: 'eBay account deletion received', ar: 'تم استلام إشعار حذف حساب eBay' },
  CATEGORY_UPDATED: { de: 'Kategorie bearbeitet', en: 'Category updated', ar: 'تعديل الفئة' },
  CHANNEL_LISTING_ENABLED: { de: 'Channel-Listing aktiviert', en: 'Channel listing enabled', ar: 'تفعيل عرض القناة' },
  EBAY_LISTING_CLEANUP_FOR_REGROUP: { de: 'eBay-Listing für Neugruppierung bereinigt', en: 'eBay listing cleaned up for regroup', ar: 'تنظيف عرض eBay لإعادة التجميع' },
  EBAY_LISTING_DISABLED: { de: 'eBay-Listing deaktiviert', en: 'eBay listing disabled', ar: 'إلغاء تفعيل عرض eBay' },
  EBAY_LISTING_ENABLED: { de: 'eBay-Listing aktiviert', en: 'eBay listing enabled', ar: 'تفعيل عرض eBay' },
  EBAY_LISTING_PUBLISHED: { de: 'eBay-Listing veröffentlicht', en: 'eBay listing published', ar: 'نشر عرض eBay' },
  EBAY_LISTING_REJECTED: { de: 'eBay-Listing abgelehnt', en: 'eBay listing rejected', ar: 'رفض عرض eBay' },
  EBAY_OAUTH_CONNECT_INITIATED: { de: 'eBay OAuth-Verbindung gestartet', en: 'eBay OAuth connect initiated', ar: 'بدء ربط OAuth مع eBay' },
  EBAY_OAUTH_CONNECTED: { de: 'eBay OAuth verbunden', en: 'eBay OAuth connected', ar: 'تم الربط مع eBay عبر OAuth' },
  EBAY_OAUTH_DISCONNECTED: { de: 'eBay OAuth getrennt', en: 'eBay OAuth disconnected', ar: 'فصل OAuth عن eBay' },
  EBAY_OAUTH_REVOKED: { de: 'eBay OAuth widerrufen', en: 'eBay OAuth revoked', ar: 'تم إبطال OAuth مع eBay' },
  EBAY_PRODUCT_GROUP_PUBLISHED: { de: 'eBay Produkt-Gruppe veröffentlicht', en: 'eBay product group published', ar: 'نشر مجموعة منتجات eBay' },
  EBAY_PUBLISH_PENDING_BATCH: { de: 'eBay Veröffentlichungs-Batch ausstehend', en: 'eBay publish-pending batch', ar: 'دفعة نشر eBay قيد الانتظار' },
