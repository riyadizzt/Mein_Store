...
import { ChannelRevenueCard } from '@/components/admin/channel-revenue-card'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {canRevenue && data?.cancellationRate && (
          <KpiCard
            title={locale === 'ar' ? 'نسبة الإلغاء' : locale === 'en' ? 'Cancellation Rate' : 'Stornoquote'}
            value={`${data.cancellationRate.rate}%`}
            subtitle={`${data.cancellationRate.cancelled}/${data.cancellationRate.total} (30 ${locale === 'ar' ? 'يوم' : locale === 'en' ? 'days' : 'Tage'})`}
            alert={Number(data.cancellationRate.rate) > 10}
          />
        )}
      </div>
      ...
      {canRevenue && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            {data?.todayByChannel?.length > 0 && (
              <ChannelRevenueCard data={data.todayByChannel} locale={locale} />
            )}
          </div>
        </div>
      )}
